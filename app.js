/* ========== app.js – DCAS CPG 2025 (REFACTORED – STABLE) ========== */
(function(){
"use strict";
...
// ============================================================================
// GLOBAL VARIABLES (will be initialised in boot)
// ============================================================================
let chapterData = window.CPG_DATA;               // may be undefined
let isChapterMissing = !chapterData;
const dom = {
...
// ============================================================================
// RENDER HELPER (replaces innerHTML override)
// ============================================================================
...
// ============================================================================
// LAST VISITED (single implementation)
// ============================================================================
function recordLastVisited() {
    ...
    try {
        ...
        list = list.filter(i => i.id !== item.id);   // remove duplicate
        list.unshift(item);
        list = list.slice(0, 5);
...
const render = {
    ...
    summary() {
        // cleanup flashcard listener if any
        ...
        utils.safeScrollTop();
        initScrollProgress();
        if (document.querySelector('.bottom-nav')) initBottomNav();
        if (chapterData && chapterData.id === 'c-index') initIndexSearch();
    },
    ...
    _renderFlashcard() {
        ...
        requestAnimationFrame(() => sizeFlashcardScene());
        const _fcResize = () => sizeFlashcardScene();
        window.addEventListener('resize', _fcResize);
        render._fcResizeCleanup = () => window.removeEventListener('resize', _fcResize);
        if (document.querySelector('.bottom-nav')) initBottomNav();
        utils.safeScrollTop();
    },
...
// Helper functions used inside render
...
document.addEventListener('click', function(e) {
    ...
    // Ripple effect (optional, keep as is)
    try {
...
document.addEventListener('DOMContentLoaded', function initApp() {
    // 1. Apply theme and font size from localStorage
    ...
    // 2. Theme toggle button (if present)
    ...
    if (themeBtn) {
        themeBtn.addEventListener('click', () => {
            ...
            initBatteryIndicator();  // refresh visibility
        });
    }
